	...
	public dynamic class stats_1 extends MovieClip
	{
		...
		public function buildTabs(tabState:Number, initializeTabs:Boolean = false) : *
		{
			...
			switch(tabState)
			{
				...
				case 2:
					...
					{
						//if(i != 4)
						//CharacterExpansionLib: Making sure the tags button is added.
		...
		public function ClickTab(tabIndex:Number) : *
		{
			...
			if(this.currentOpenPanel != tabIndex)
			{
				...
				this.panelBg1_mc.visible = tabIndex < 5 || tabIndex == 6 || tabIndex == 8; // CharacterExpansionLib change
		...
		public function INTSetWarnAndPoints(index:Number, pointsValue:Number) : *
		{
			...
			else
			{
				...
				// }
				//CharacterExpansionLib: Fix to make sure all the buttons are hidden.
				var i:uint = 0;
				while(i < this.pointsWarn.length)
				{
					this.pointsWarn[i].visible = false;
		...
		{
			//CharacterExpansionLib: Allowing the custom stats panel to display available points.
		...
		public function frame1() : *
		{
			...
			//CharacterExpansionLib - Making the main stats scrollable
			...
			//CharacterExpansionLib additions
